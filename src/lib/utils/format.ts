...
export function formatAsKey(input: unknown) {
  ...
  if (typeof input !== 'string') {
    throw new AkeneoError(`The [input] parameter must be a string`)
  }
  ...
  if (output.length < 2) {
    throw new AkeneoError(`Formatted key did not meet minimum length of 2 characters: ${output}`)
  }
  if (output.length > 256) {
    throw new AkeneoError(`Formatted key exceeds the maximum length of 256 characters: ${output}`)
  }
